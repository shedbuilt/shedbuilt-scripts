...
SHDREL_IMGFILE="$6"
SHDREL_WORKDIR="${7%/}"
SHDREL_CONFIGSCRIPT=$(readlink -f -n "$8")
...
# Install Palooza Files
cd "$SHDREL_WORKDIR" &&
tar xvhf palooza.tar.xz -C "$SHDREL_MOUNT"/root || exit 1

# Run post-install configuration script in chroot
chroot "$SHDREL_MOUNT" /usr/bin/env -i \
    HOME=/root                  \
    TERM="$TERM"                \
    PS1='\u:\w\$ '              \
